...
class FFreeformVBO : public FVertexBuffer
{
public:
...
class FFreeformIBO : public FIndexBuffer
{
public:
...
class FFreeformVertexFactory : public FLocalVertexFactory
{
public:

	FFreeformVertexFactory()
	{
	}

	void Init(const FFreeformVBO* VertexBuffer)
	{
		ENQUEUE_UNIQUE_RENDER_COMMAND_TWOPARAMETER(
			InitFreeformVertexFactory,
			FFreeformVertexFactory*, VertexFactory, this,
			const FFreeformVBO*, VertexBuffer, VertexBuffer,
			{
...
class FFreeformMeshSceneProxy : public FPrimitiveSceneProxy
{
public:

	FFreeformMeshSceneProxy(UFreeformMeshComponent* Component) :
		FPrimitiveSceneProxy(Component),
		MaterialRelevance(Component->GetMaterialRelevance(GetScene().GetFeatureLevel()))
	{
		for (int TriIdx = 0; TriIdx<Component->Triangles.Num(); TriIdx++)
		{
			FFreeformMeshTriangle& Triangle = Component->Triangles[TriIdx];
	...
	virtual ~FFreeformMeshSceneProxy()
	{
...
private:
	...
	FFreeformVBO VertexBuffer;
	FFreeformIBO IndexBuffer;
	FFreeformVertexFactory VertexFactory;

	FMaterialRelevance MaterialRelevance;
};

void UFreeformMeshComponent::SetTriangles(const TArray<FFreeformMeshTriangle>& aTriangles)
{
...
void UFreeformMeshComponent::AddTriangles(const TArray<FFreeformMeshTriangle>& aTriangles)
{
	Triangles.Append(aTriangles);
	MarkRenderStateDirty();
}

void UFreeformMeshComponent::ClearTriangles()
{
	Triangles.Reset();
	MarkRenderStateDirty();
}

FPrimitiveSceneProxy* UFreeformMeshComponent::CreateSceneProxy()
{
	FPrimitiveSceneProxy* Proxy = nullptr;
	if (Triangles.Num() > 0)
	{
		Proxy = new FFreeformMeshSceneProxy(this);
	}
	return Proxy;
}

int32 UFreeformMeshComponent::GetNumMaterials() const
{
	return 1;
}

FBoxSphereBounds UFreeformMeshComponent::CalcBounds(const FTransform & LocalToWorld) const
{
	if (Triangles.Num() > 0)
	{
...
bool UFreeformMeshComponent::GetPhysicsTriMeshData(struct FTriMeshCollisionData* CollisionData, bool InUseAllTriData)
{
	FTriIndices Triangle;
	for (int32 i = 0;  i < Triangles.Num(); i++)
	{
		const FFreeformMeshTriangle& CurrentTriangle = Triangles[i];
...
bool UFreeformMeshComponent::ContainsPhysicsTriMeshData(bool InUseAllTriData) const
{
	return (Triangles.Num() > 0);
}

void UFreeformMeshComponent::UpdateBodySetup()
{
...
void UFreeformMeshComponent::UpdateCollision()
{
...
UBodySetup* UFreeformMeshComponent::GetBodySetup()
{
	UpdateBodySetup();
