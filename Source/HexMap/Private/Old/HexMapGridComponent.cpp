	...
	if (ChunkIt)
	{
		...
		if (HexMapTileLocationComponentIt)
		{
			...
			for (UHexMapTileLocationComponent* HexMapTileLocationComponent : HexMapTileLocationComponent->Neighbours)
			{
				...
				if (FMath::Abs(TileHeight - TileLocation.Z) > TileHeightThreshold)
				{
			...
			if (HexMapTileLocationComponentIt)
			{
				...
				if (InvalidatedNeighbours.Num() >= ReqLocationsNum - 1)
				{
					if (!bIgnoreHeight)
					{
						...
						{
							if (FMath::Abs(TileHeight - HexMapTileLocationComponent->GetComponentLocation().Z) > TileHeightThreshold)
							{
								bIsHeightSame = false;
								break;
							}
